...
import { n as buildPluginCompatibilityNotices, o as formatPluginCompatibilityNotice } from "./status-DE_JvlNY.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-DbHR1Fs0.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-D6OED1iB.js");
		const { logConfigUpdated } = await import("./logging-dpNOqU8L.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-BY3t-mHt.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-D_yWI-I2.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-RtfT61us.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-Dbu2IRkg.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CsCtjKxQ.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-C10um5ax.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-BsqJQjTC.js");
		const { setupChannels } = await import("./onboard-channels-B9oG-TKQ.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-C2r5oTPW.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-Bh6nTGpe.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-DTu8XQOu.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-sZvVGM89.js");
